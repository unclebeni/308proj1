...
int main() {
  ...
  scheduler schedulers[NUM_SCHEDULERS];int finished;        /* Convenience - indicates if the process has finished */
...
int first_come_first_served(process proc[], int t)
{
  ...
  for(i = 0; i < NUM_PROCESSES ; i++)
  {
	...
	{
	  if(proc[i].arrivaltime < shortestTime)
	  {
	    shortestTime = proc[i].arrivaltime;
	    index = i;
	  }
	}
  }
...
{ 
  ...
  for(i = lastIndex; i < NUM_PROCESSES + lastIndex; i++)
  {
	index = ((i%NUM_PROCESSES)+1)%NUM_PROCESSES;
	if(proc[index].arrivaltime <= t)
	{
		if(proc[index].finished == 0)
		{
		  lastIndex = index;
		  return index;
		}
...
int round_robin_priority(process proc[], int t)
{
 ...
 int index = -1;
 int i, j, k;
 //Loop for high priority(level 2)
for(i = prevProc; i<NUM_PROCESSES + prevProc; i++)
  {
     index = ((i%NUM_PROCESSES)+1)%NUM_PROCESSES;
     if((proc[index].arrivaltime <= t) && (proc[index].finished == 0) && (proc[index].priority == 2))
     {
	prevProc = 0;
	return index;
     }
 ...
 //Loop for medium priority(level 1)
  for(i = prevProc; i<NUM_PROCESSES + prevProc; i++)
  {
     index = ((i%NUM_PROCESSES)+1)%NUM_PROCESSES;
     if((proc[index].arrivaltime <= t) && (proc[index].finished == 0) && (proc[index].priority == 1))
     {
	prevProc = 0;
	return index;
     }
 ...
 //Loop for low priority (level 0) 
 for(i = prevProc; i<NUM_PROCESSES + prevProc; i++)
  {
     index = ((i%NUM_PROCESSES)+1)%NUM_PROCESSES;
     if((proc[index].arrivaltime <= t) && (proc[index].finished == 0) && (proc[index].priority == 0))
     {
	prevProc = 0;
	return index;
     }
